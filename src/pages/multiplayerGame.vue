...
export default {
    ...
    setup() {
        ...
        function eventsOn() {
            ...
            //接收app的wsclient
            eventBus.on("multiplayerViewGetWsClient", (data) => {
                wsSocket = data;
                // console.log(wsSocket);
            });
            eventBus.on("routeToFinalPage", (state) => {
                if (state == STATE.GAME_STATE_WIN) {
                    router.push({
                        name: "gamewin",
                        path: "/gamewinpage",

                    });
                } else if (state == STATE.GAME_STATE_LOSE) {
                    router.push({
                        name: "gamelose",
                        path: "/gamelosepage",
                    });
        ...
        function eventsOff() {
            ...
            eventBus.off("multiplayerViewGetWsClient");
            eventBus.off("multiplayerViewGetWsClient");
        }
        //键盘事件监听器
        ...
        function initData() {
            //获取app中的wsclient连接
            eventBus.emit("sendAppWsClient", "multiplayer");
            gameInstance = wsSocket.gameInstance;
            console.log("multi gameInstance", gameInstance);
            //初始化视窗
            initScreen(gameInstance);
            //初始化对象
            multiplayInitObject(gameInstance);
        ...
        function startGame(gameInstance) {
            ...
            if (wsSocket) {
                ...
                gameInstance.onlineGameLoopId = setInterval(() => {
                    onlineGameLoop(gameInstance, true);
                }, 20);
            }
        }
        function clientIsReady() {
            const content = new SocketMessage(
                ...
                new MultiMsg(
                    ...
                    MULTI_CLIENT_TYPE.MULTI_CLIENT_READY
                )
            );
            //发送到服务器
        ...
        onBeforeUnmount(() => {
            ...
            clearTimer();
            console.log('multiplayerGame clear');
        });
        return {
            gameInstance,
        };
